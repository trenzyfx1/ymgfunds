...
// Reads referral code from URL (?ref=CODE) OR from the input field
...
function getPremiumReferralCode() {
  const params = new URLSearchParams(window.location.search);
  const fromURL = params.get("pref"); // ?pref=YMGP-XXXXXX
  if (fromURL) return fromURL.trim();
...
async function handleReferral(referredBy, newUser, userName) {
  ...
  try {
    ...
    if (snap.empty) {
      console.warn("❌ Invalid referral code:", referredBy);
      return;
    }
    ...
    // Add pending referral record under referrer
    ...
    console.log("✅ Referral recorded for", referrerId);
  } catch (err) {
...
if (refInp) { refInp.style.borderColor = ""; refInp.style.boxShadow = ""; }
  ...
  if (refVal) {
    ...
    const refQuery = query(
      collection(db, "users"),
      where("referralCode", "==", refVal)
    );
    const refSnap = await getDocs(refQuery);

    // Also check premium referral codes
    const premQuery = query(
      collection(db, "users"),
      where("premiumReferralCode", "==", refVal)
    );
    const premSnap = await getDocs(premQuery);

    if (refSnap.empty && premSnap.empty) {
      const refErrEl = document.getElementById("referralError");
      if (refErrEl) {
        refErrEl.textContent = "Invalid referral code. Please check and try again.";
      }
  ...
  try {
    ...
    const referredBy = getReferralCode(); // ← reads from input OR URL
    ...
    await setDoc(doc(db, "users", user.uid), {
      ...
      premiumReferredBy: getPremiumReferralCode() || null,
