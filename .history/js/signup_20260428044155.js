...
async function validateReferralCode(refVal) {
  if (!refVal) return true; // empty is fine

  // Basic format check first — no Firestore needed
  const isNormalFormat  = /^YMG-[A-Z0-9]{6}$/.test(refVal);
  const isPremiumFormat = /^YMGP-[A-Z0-9]{6}$/.test(refVal);

  if (!isNormalFormat && !isPremiumFormat) {
    return false; // wrong format immediately
  }

  // Now check Firestore — wrapped in try/catch so it never blocks signup
  try {
    if (isNormalFormat) {
      const q    = query(collection(db, "users"), where("referralCode", "==", refVal));
      const snap = await getDocs(q);
      return !snap.empty;
    } else {
      const q    = query(collection(db, "users"), where("premiumReferralCode", "==", refVal));
      const snap = await getDocs(q);
      return !snap.empty;
    }
  } catch (err) {
    // If Firestore check fails (e.g. rules block it), allow signup to proceed
    console.warn("Referral check skipped:", err.code);
    return true;
  }
}
...
document.getElementById("createBtn").addEventListener("click", async () => {
  ...
  if (refVal) {
    const isValid = await validateReferralCode(refVal);
    if (!isValid) {
      const refErrEl = document.getElementById("referralError");
      if (refErrEl) refErrEl.textContent = "Invalid referral code. Please check and try again.";
      if (refInput) {
  ...
  try {
    ...
    await handleReferral(referredBy, user, name);
...
if (googleBtn) {
  googleBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      await handleReferral(referredBy, user, user.displayName);
