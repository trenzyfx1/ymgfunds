...
function validateReferralFormat(refVal) {
  ...
  return /^YMG-[A-Z0-9]{6}$/.test(refVal) || /^YMGP-[A-Z0-9]{6}$/.test(refVal);
}

// Non-blocking referral handler — runs after user reaches dashboard
// so it doesn't slow down signup at all
function handleReferralAsync(referredBy, newUser, userName) {
...
document.getElementById("createBtn").addEventListener("click", async () => {
  ...
  if (refVal && !validateReferralFormat(refVal)) {
    ...
    if (refInput) { refInput.style.borderColor = "#dc2626"; refInput.style.boxShadow = "0 0 0 3px rgba(220,38,38,0.1)"; }
    return;
  }
  ...
  try {
    ...
    handleReferralAsync(refBy, user, name);
...
if (googleBtn) {
  googleBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const settingsSnap = await getDoc(doc(db, "config", "platformSettings"));
      const settings     = settingsSnap.exists() ? settingsSnap.data() : {};
      if (settings.maintenanceMode) { alert("The platform is under maintenance. Please try again later."); await auth.signOut(); return; }
      if (settings.registrationsEnabled === false) { alert("New registrations are temporarily disabled."); await auth.signOut(); return; }

      const userRef  = doc(db, "users", user.uid);
      const userSnap = await getDoc(userRef);

      if (userSnap.exists()) {
        // Returning Google user — just log them in, don't overwrite anything
      ...
      handleReferralAsync(refBy, user, user.displayName);
