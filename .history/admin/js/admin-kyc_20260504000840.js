...
function renderTable(data) {
  ...
  if (!page.length) { tbody.innerHTML = `<tr><td colspan="7" class="adm-table-empty">No users found.</td></tr>`; return; }
      ...
      <tr>
        ...
        <td>
          <div style="display:flex;gap:6px;">
            <button class="adm-action-icon-btn blue" onclick="viewKYC('${u.id}')" title="View & Manage"><i class="fa-solid fa-eye"></i></button>
...
window.closeKycModal = function() { document.getElementById("kycDetailModal").classList.remove("active"); };
document.getElementById("kycModalClose")?.addEventListener("click", closeKycModal);
document.getElementById("kycDetailModal")?.addEventListener("click", e => { if (e.target.id === "kycDetailModal") closeKycModal(); });
...
document.getElementById("kycPrevBtn")?.addEventListener("click", () => { if (currentPage > 1) { currentPage--; renderTable(getFiltered()); } });
document.getElementById("kycNextBtn")?.addEventListener("click", () => { if (currentPage * PAGE_SIZE < getFiltered().length) { currentPage++; renderTable(getFiltered()); } });

requirePagePassword("KYC Verification").then(() => loadKYC());