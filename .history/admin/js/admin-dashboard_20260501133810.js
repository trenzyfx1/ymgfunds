...
async function loadDashboard() {
  try {
    ...
    for (const user of users) {
      ...
      txSnap.forEach(tx => {
        const t    = tx.data();
        const tStr = t.date?.seconds
          ? new Date(t.date.seconds * 1000).toLocaleDateString("en-GB")
          : "";
        const tMonthStr = t.date?.seconds
        ...
        if (t.type === "deposit") {
          ...
          if (recentDeposits.length < 8) {
            recentDeposits.push({ ...t, userName: user.name, userId: user.id });
          }
        }
    ...
    const loanSnap = await getDocs(
      query(collection(db, "loanRequests"), where("status", "==", "pending"))
    );
    ...
    <tr>
      <td>
        <div class="adm-user-cell">
          ...
          <span class="adm-user-name">${d.userName || "—"}</span>
        </div>
      </td>
...
function renderWithdrawalsTable(withdrawals) {
  ...
  tbody.innerHTML = withdrawals.map(w => `
    <tr>
      <td>
        <div class="adm-user-cell">
          ...
          <div>
            ...
            <p class="adm-user-email">${w.phone || ""}</p>
          </div>
        </div>
      </td>
      ...
      <td>${w.method || "—"}</td>
      <td>
        <a href="withdrawals.html" class="adm-approve-btn">Review</a>
      </td>
    </tr>
  `).join("");
}
...
function renderUsersTable(users) {
  ...
  tbody.innerHTML = users.map(u => `
    <tr>
      ...
      <td>
        <span class="adm-badge ${u.emailVerified ? "active" : "pending"}">
...
document.getElementById("admRefresh")?.addEventListener("click", () => {
  loadDashboard();
});
