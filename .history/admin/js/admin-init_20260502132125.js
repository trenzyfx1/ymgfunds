...
async function loadAdminNotifications() {
  ...
  try {
    ...
    // Then fetch all alerts — use getDocs to avoid index requirement
    let notifs = [];
    try {
      const snap = await getDocs(
        query(collection(db, "adminNotifications"), orderBy("createdAt", "desc"), limit(40))
      );
      notifs = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    } catch {
      // If index not ready yet, fetch without ordering
    ...
    if (list) list.innerHTML = `<div style="text-align:center;padding:20px;color:var(--adm-muted);font-size:0.82rem;">Failed to load alerts. Please refresh.</div>`;
  }
}
...
async function scanPlatformForAlerts() {
  try {
    ...
    try {
      ...
      existingSnap = await getDocs(collection(db, "adminNotifications"));
    }
