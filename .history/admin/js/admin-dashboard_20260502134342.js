  ...
  try {
    ...
    users.forEach(u => {
      const dayKey = u.createdAt?.seconds
        ? new Date(u.createdAt.seconds * 1000).toLocaleDateString("en-GB", { day: "2-digit", month: "short" })
        : null;
      if (dayKey) allUsersByDay[dayKey] = (allUsersByDay[dayKey] || 0) + 1;
      if (!u.emailVerified && !u.authEmailVerified && !u.phoneVerified) unverifiedKyc++;
    });
    ...
    const userDataPromises = users.map(async (user) => {
      ...
      txSnap.forEach(tx => {
        ...
        if (t.type === "deposit") {
          ...
          // Use short key (dd Mon) to match chart labels
          if (tDate) {
            const shortKey = tDate.toLocaleDateString("en-GB", { day: "2-digit", month: "short" });
            allDepositsByDay[shortKey] = (allDepositsByDay[shortKey] || 0) + (t.amount || 0);
          }
        ...
        if (t.type === "withdrawal") {
          ...
          if (tDate) {
            const shortKey = tDate.toLocaleDateString("en-GB", { day: "2-digit", month: "short" });
            allWithdrawalsByDay[shortKey] = (allWithdrawalsByDay[shortKey] || 0) + (t.gross || t.amount || 0);
          }
        }
      ...
      invSnap.forEach(inv => {
        const i    = inv.data();
        const raw  = i.planName || i.plan || "";
        // Normalize plan name — match even if slightly different
        const plan = Object.keys(planCounts).find(k => k.toLowerCase() === raw.toLowerCase()) || raw;
        if (i.status === "active" || i.status === "matured") {
          totalInvested += i.amount || 0;
          activePlansCount++;
          // Use GHS amount for pie chart (not count)
          if (planCounts[plan] !== undefined)  planCounts[plan]  += (i.amount || 0);
          else                                 planCounts[plan]   = (i.amount || 0);
          if (planProfits[plan] !== undefined) planProfits[plan] += (i.profitEarned || 0);
          else                                planProfits[plan]   = (i.profitEarned || 0);
        }
      });
    });
...
function renderPlanChart(planCounts) {
  ...
  // Only show plans with data
  const entries = Object.entries(planCounts).filter(([, v]) => v > 0);
  ...
  planChartInstance = new Chart(ctx, {
    type: "doughnut",
    data: { labels, datasets: [{ data, backgroundColor: colors.slice(0, data.length), borderColor: "transparent", borderWidth: 0, hoverOffset: 6 }] },
    options: {
      responsive: true,
      cutout: "72%",
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (ctx) => {
              ...
              return ` ${ctx.label}: GHS ${val.toLocaleString("en-GH", { minimumFractionDigits: 2 })} (${pct}%)`;
            }
          }
        }
      }
  ...
  if (legend) {
    if (!labels.length) {
      legend.innerHTML = `<p style="text-align:center;color:var(--adm-muted);font-size:0.78rem;padding:8px 0;">No active investments yet.</p>`;
      return;
    }
    legend.innerHTML = labels.map((label, i) => {
      const pct = total ? Math.round((data[i] / total) * 100) : 0;
      const amt = "GHS " + data[i].toLocaleString("en-GH", { minimumFractionDigits: 2 });
      return `<div class="adm-legend-item"><div class="adm-legend-left"><div class="adm-legend-dot" style="background:${colors[i]}"></div><span>${label}</span></div><div style="text-align:right;"><span class="adm-legend-pct">${pct}%</span><br><span style="font-size:0.68rem;color:var(--adm-muted);">${amt}</span></div></div>`;
    }).join("");
  }
}
...
function renderProfitChart(planProfits) {
  ...
  const entries = Object.entries(planProfits).filter(([, v]) => v > 0);
  const labels  = entries.map(([k]) => k);
  const data    = entries.map(([, v]) => v);
  const colors  = ["#22c55e", "#c9a84c", "#3b82f6", "#a855f7", "#f97316", "#ef4444"];
  const total   = data.reduce((s, v) => s + v, 0);
  ...
  if (legend) {
    if (!labels.length) {
      legend.innerHTML = `<p style="text-align:center;color:var(--adm-muted);font-size:0.78rem;padding:8px 0;">No profits recorded yet.</p>`;
      return;
    }
    legend.innerHTML = labels.map((label, i) => {
      const pct = total ? Math.round((data[i] / total) * 100) : 0;
