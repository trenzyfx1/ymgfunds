...
await requirePagePassword("Admin Settings");
...
window.saveToggle = async function(key, value) {
  ...
  const confirmed = await confirmWithPassword(`${value ? "Enable" : "Disable"} ${label}`);
  if (!confirmed) {
...
async function loadAdminProfile() {
  try {
    ...
    const pwFields = ["sttCurrentPw", "sttNewPw", "sttConfirmPw"];
    pwFields.forEach(id => {
      const el = document.getElementById(id);
      if (el) {
        el.value          = "";
