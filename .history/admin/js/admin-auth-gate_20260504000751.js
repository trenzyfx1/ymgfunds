...
import {
  reauthenticateWithCredential, EmailAuthProvider, onAuthStateChanged
...
export function requirePagePassword(pageLabel) {
  return new Promise((resolve) => {
    const showGate = (user) => {
      const overlay = document.createElement("div");
      ...
      pwInput.addEventListener("focus", () => { pwInput.style.borderColor = "#c9a84c"; });
      pwInput.addEventListener("blur",  () => { pwInput.style.borderColor = "rgba(255,255,255,0.08)"; });
