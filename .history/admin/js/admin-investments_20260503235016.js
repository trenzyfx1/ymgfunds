...
await requirePagePassword("Investment Management");
...
function getReturnRate(inv) { const rate = inv.returnRate ?? inv.rate ?? 0; return (rate * 100).toFixed(1); }
...
function getMaturitySeconds(inv) { return inv.maturityDate?.seconds || inv.maturityDate?._seconds || null; }
function getStartSeconds(inv)    { return inv.startDate?.seconds    || inv.startDate?._seconds    || null; }
