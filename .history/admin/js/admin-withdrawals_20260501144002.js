...
window.viewDetails = function(id) {
  ...
  body.innerHTML = `
    <div class="adm-detail-grid">
      ...
      <div class="adm-detail-item"><span>Payment Details</span><strong style="white-space:pre-line;font-size:0.82rem;">${w.paymentDetails || "—"}</strong></div>
...
window.approveWithdrawal = async function(id) {
  ...
  const confirmed = confirm(`Approve withdrawal of ${fmtGHS(w.gross || w.amount)} for ${w.name}?\n\nNet amount to send: ${fmtGHS(w.amount)}\nMethod: ${w.method}\n\nThis will mark the request as completed. Make sure you have manually sent the funds.`);
  if (!confirmed) return;
  ...
  try {
    ...
    const userTxQuery = await getDocs(collection(db, "users", w.uid, "transactions"));
    userTxQuery.forEach(async (txDoc) => {
    ...
    await createNotification(
      ...
      `Your withdrawal of ${fmtGHS(w.gross || w.amount)} has been processed and sent to your ${w.method}. Reference: ${w.reference}`
    );

    const idx = allWithdrawals.findIndex(x => x.id === id);
    if (idx !== -1) allWithdrawals[idx].status = "completed";

    renderTable(getFilteredData());
    showToast(`Withdrawal approved for ${w.name}.`, "success");
...
window.confirmReject = async function(id) {
  ...
  try {
    ...
    await updateDoc(doc(db, "users", w.uid), {
      balance: (w.grossAmount || w.gross || w.amount || 0)
    });
    ...
    await createNotification(
      ...
      "withdrawal",
      "Withdrawal Rejected ❌",
      `Your withdrawal request of ${fmtGHS(w.gross || w.amount)} has been rejected. Reason: ${reason}. Your balance has been refunded. Reference: ${w.reference}`
    );
...
function getFilteredData() {
  ...
  return allWithdrawals.filter(w => {
    ...
    const matchSearch = !search || (w.name || "").toLowerCase().includes(search) || (w.reference || "").toLowerCase().includes(search) || (w.email || "").toLowerCase().includes(search);
