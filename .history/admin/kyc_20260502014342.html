  ...
  tbody.innerHTML = page.map(u => {
    ...
    return `
      <tr>
        ...
        <td>
          <span class="adm-badge ${u.emailVerified ? "completed" : "failed"}">
            ${u.emailVerified ? "✓ Verified" : "✗ Not Verified"}
          </span>
        </td>
        <td>
          <span class="adm-badge ${u.phoneVerified ? "completed" : "failed"}">
        ...
        <td>
          <div style="display:flex;gap:6px;">
            ...
            ${!u.emailVerified ? `
              <button class="adm-action-icon-btn green" onclick="manualVerifyEmail('${u.id}')" title="Manually verify email">
                <i class="fa-solid fa-envelope-circle-check"></i>
              </button>
            ` : ""}
            ${!u.phoneVerified ? `
              <button class="adm-action-icon-btn gold" onclick="manualVerifyPhone('${u.id}')" title="Manually verify phone">
...
window.viewKYC = function(uid) {
  ...
  body.innerHTML = `
    ...
    <div class="adm-detail-grid">
      ...
      <div class="adm-detail-item">
        ...
        <span class="adm-badge ${u.emailVerified ? "completed" : "failed"}">${u.emailVerified ? "✓ Verified" : "✗ Not Verified"}</span>
      </div>
      <div class="adm-detail-item">
        <span>Phone Verification</span>
        <span class="adm-badge ${u.phoneVerified ? "completed" : "failed"}">${u.phoneVerified ? "✓ Verified" : "✗ Not Verified"}</span>
      </div>
  ...
  const actions = [];
  if (!u.emailVerified) actions.push(`<button class="adm-btn green" onclick="manualVerifyEmail('${uid}');closeKycModal();"><i class="fa-solid fa-envelope-circle-check"></i> Verify Email Manually</button>`);
  if (!u.phoneVerified) actions.push(`<button class="adm-btn blue"  onclick="manualVerifyPhone('${uid}');closeKycModal();"><i class="fa-solid fa-phone-volume"></i> Verify Phone Manually</button>`);
...
window.manualVerifyEmail = async function(uid) {
  const u = allUsers.find(x => x.id === uid);
  if (!u) return;
  const confirmed = confirm(`Manually mark email as verified for ${u.name}?`);
  if (!confirmed) return;
  try {
...
window.manualVerifyPhone = async function(uid) {
  const u = allUsers.find(x => x.id === uid);
  if (!u) return;
  const confirmed = confirm(`Manually mark phone as verified for ${u.name}?`);
  if (!confirmed) return;
  try {
...
window.revokeEmailVerification = async function(uid) {
  const u = allUsers.find(x => x.id === uid);
  if (!u) return;
  const confirmed = confirm(`Revoke email verification for ${u.name}? They will need to re-verify their email.`);
  if (!confirmed) return;
  try {
    await updateDoc(doc(db, "users", uid), { emailVerified: false });
...
window.revokePhoneVerification = async function(uid) {
  const u = allUsers.find(x => x.id === uid);
  if (!u) return;
  const confirmed = confirm(`Revoke phone verification for ${u.name}? They will need to re-verify their phone.`);
  if (!confirmed) return;
  try {
