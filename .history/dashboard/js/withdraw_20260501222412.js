...
document.getElementById("wdrSubmitBtn").addEventListener("click", async () => {
  ...
  if (todayTotal >= WITHDRAW_MAX) { errEl.textContent = `Daily limit of GHS ${WITHDRAW_MAX.toLocaleString()} reached. You have already withdrawn ${fmtGHS(todayTotal)} today.`; return; }
...
document.getElementById("wdrChangeAcctClose")?.addEventListener("click", () => {
  document.getElementById("wdrChangeAcctModal").classList.remove("inv-modal-active");
});
document.getElementById("wdrChangeAcctModal")?.addEventListener("click", (e) => {
  if (e.target.id === "wdrChangeAcctModal") document.getElementById("wdrChangeAcctModal").classList.remove("inv-modal-active");
});
...
document.getElementById("wdrVerifyChangeOtpBtn")?.addEventListener("click", async () => {
  ...
  try {
    await updateDoc(doc(db, "users", WDR_USER.uid), {
      withdrawalAccountLocked:  false,
      savedWithdrawalAccount:   null
    });
