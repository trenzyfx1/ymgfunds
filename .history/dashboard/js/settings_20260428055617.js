...
onAuthStateChanged(auth, async (user) => {
  ...
  await loadNotifPrefs(user.uid); // ← ADD THIS LINE HERE
...
async function loadNotifPrefs(uid) {
  try {
    ...
    NOTIF_KEYS.forEach(key => {
      ...
      if (!el) return;
      // Default is true for all except notifPromo
      el.checked = key in prefs ? prefs[key] : key !== "notifPromo";
    });
  } catch (err) {
    console.error("Load notif prefs error:", err);
  }
}
...
// ═══════════════════════════════════════════════════════
// PASTE THIS AT THE BOTTOM OF settings.js
// ═══════════════════════════════════════════════════════
...
async function loadDisplaySettings(uid) {
  try {
    ...
    // Apply hide balance immediately if set
...
document.getElementById("dispHideBalance")?.addEventListener("change", (e) => {
  applyHideBalance(e.target.checked);
});

document.getElementById("dispCompact")?.addEventListener("change", (e) => {
...
// ── LOAD ALL NEW SETTINGS after auth ──────────
// Add these three lines inside your onAuthStateChanged
// right after:  await loadNotifPrefs(user.uid);
//