...
let INV_LOAN_PLAN = null; // tracks which loan plan is being applied for
...
function invRefreshUI() {
  // Standard strips — now includes loan card
  const stdStrips  = ["istrip-starter", "istrip-fixed", "istrip-growth", "istrip-std-loan"];
  const premStrips = ["istrip-p182", "istrip-p365", "istrip-p3yr", "istrip-prem-loan"];
...
document.getElementById("pwModalConfirm").addEventListener("click", async () => {
  ...
  try {
    ...
    if (INV_PW_ACTION === "activate") {
      ...
      // Open loan modal after password confirmed
...
// ══════════════════════════════════════════════
// INVEST + LOAN BUTTONS — password first
// ══════════════════════════════════════════════
document.querySelectorAll(".inv-cta-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    const plan = btn.dataset.plan;
    const tier = btn.dataset.tier;
    const type = btn.dataset.type; // "loan" or undefined

    // If locked, ask to activate first
...
// ── Activation banner buttons — UNCHANGED ──────
...
// ── Activation modal close — UNCHANGED ─────────
...
document.getElementById("actModalConfirm").addEventListener("click", () => {
  ...
  const handler = window.PaystackPop.setup({
    key:      "pk_live_bbf9be7a02b476503f3608a1a30ca24114186c7d",
...
// ── Referral copy button — UNCHANGED ───────────
