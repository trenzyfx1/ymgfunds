...
document.getElementById("verifyEmailBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    // Poll every 5s — auto-update UI when user verifies
...
// Confirm OTP — replace isValid line with real SMS API when ready
document.getElementById("confirmOtpBtn").addEventListener("click", async () => {
  ...
  document.getElementById("confirmOtpBtn").addEventListener("click", async () => {
    ...
    if (otp.length < 6) {
      document.getElementById("otpError").textContent = "Enter full code";
      return;
    }
...
document.getElementById("forgotPwBtn").addEventListener("click", async () => {
  ...
  try {
    const actionCodeSettings = {
      url: window.location.origin + "/dashboard/settings.html",
      handleCodeInApp: false
    };
