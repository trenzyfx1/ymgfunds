...
onAuthStateChanged(auth, async (user) => {
  ...
  DEP_SETTINGS = await getPlatformSettings();

  if (DEP_SETTINGS.maintenanceMode) {
    window.location.href = "../pages/maintenance.html";
    return;
  }

  if (!DEP_SETTINGS.depositsEnabled) {
    document.getElementById("depError").textContent = "Deposits are temporarily disabled. Please try again later.";
...
function updateSummary(amount) {
  ...
  const feeRate   = (DEP_SETTINGS?.depositFeeRate || 0) / 100;
  const fee       = parseFloat((amount * feeRate).toFixed(2));
  const netAmount = amount - fee;

  document.getElementById("summaryAmt").textContent = fmtGHS(amount);
  document.getElementById("summaryFee").textContent = fee > 0 ? `− ${fmtGHS(fee)} (${DEP_SETTINGS.depositFeeRate}%)` : "GHS 0.00 (Free ✓)";
  document.getElementById("summaryNet").textContent = fmtGHS(netAmount);
...
document.getElementById("depositBtn").addEventListener("click", () => {
  ...
  if (!DEP_SETTINGS?.depositsEnabled) {
    errEl.textContent = "Deposits are temporarily disabled.";
    return;
  }
  ...
  const minDep = DEP_SETTINGS?.minDeposit || 50;
  if (amount < minDep) {
    errEl.textContent = `Minimum deposit is ${fmtGHS(minDep)}.`;
    return;
  }
  ...
  const handler = window.PaystackPop.setup({
    ...
    metadata: {
      custom_fields: [
        ...
        { display_name: "Platform", variable_name: "platform", value: "YMG IQ"     }
      ]
    },
    onClose: () => {
      btn.disabled = false;
...
async function handlePaymentSuccess(amount, reference) {
  try {
    ...
    if (!uSnap.exists()) {
      await setDoc(uRef, {
        balance: netAmount, invested: 0, profit: 0, activePlans: 0,
        referralEarnings: 0, referralCount: 0, email: DEP_USER.email,
        createdAt: serverTimestamp()
      });
    } else {
      const uData = uSnap.data();
      await updateDoc(uRef, { balance: (uData.balance || 0) + netAmount });
    }

    await addDoc(collection(db, "users", DEP_USER.uid, "transactions"), {
      type: "deposit", amount: netAmount, gross: amount, fee,
      method: "Paystack", reference, status: "completed", date: serverTimestamp()
    });
...
async function handleReferralCredit(uData) {
  try {
    if (!DEP_SETTINGS?.referralsEnabled) return;
    const referredBy = uData.referredBy;
    if (!referredBy)            return;
    if (uData.referralRewarded) return;

    const REFERRAL_REWARD = DEP_SETTINGS?.referralBonus || 10;
    ...
    await addDoc(collection(db, "users", referrerId, "transactions"), {
      type: "referral_reward", amount: REFERRAL_REWARD,
    ...
    refSnap.forEach(async (refDoc) => {
      if (refDoc.data().userId === DEP_USER.uid) {
        await updateDoc(doc(db, "users", referrerId, "referrals", refDoc.id), {
          status: "completed", amountEarned: REFERRAL_REWARD, depositedAt: serverTimestamp()
        });
      }
    });

    await updateDoc(doc(db, "users", DEP_USER.uid), { referralRewarded: true });
...
async function loadDepositHistory() {
  ...
  try {
    ...
    const snap = await getDocs(q);
    const deposits = [];
    snap.forEach(ds => { const tx = ds.data(); if (tx.type === "deposit") deposits.push(tx); });
    ...
    deposits.forEach(tx => {
      const date = tx.date?.seconds
        ...
        : "—";
      const ref         = tx.reference ? `<span class="dep-ref-tag">${tx.reference}</span>` : "—";
      ...
      tbody.innerHTML  += `
        <tr>
          ...
          <td class="dep-fee-cell" style="color:#16a34a;">${tx.fee > 0 ? `− ${fmtGHS(tx.fee)}` : "Free"}</td>
