...
onAuthStateChanged(auth, async (user) => {
  ...
  onSnapshot(doc(db, "users", user.uid), async (snap) => {
    ...
    await user.reload();
    const emailVerifiedFirebase = user.emailVerified;
    const emailVerifiedFirestore = d.emailVerified === true || d.authEmailVerified === true;
    const emailVerified = emailVerifiedFirebase || emailVerifiedFirestore;
    const phoneVerified = d.phoneVerified || false;

    if (emailVerified && !d.emailVerified) {
      await updateDoc(doc(db, "users", user.uid), { emailVerified: true }).catch(() => {});
    }

    updateEmailStatus(emailVerified);
    updatePhoneStatus(phoneVerified);
    updateSecurityOverview(emailVerified, phoneVerified);
  });
  ...
  try {
    ...
    const poll = setInterval(async () => {
      ...
      if (STT_USER.emailVerified) {
        clearInterval(poll);
        await updateDoc(doc(db, "users", STT_USER.uid), { emailVerified: true });
        updateEmailStatus(true);
        updateSecurityOverview(true, verifiedPhone ? true : false);
...
document.getElementById("verifyPhoneBtn")?.addEventListener("click", async () => {
  ...
  try {
    const snap  = await getDoc(doc(db, "users", STT_USER.uid));
    const d     = snap.data();
    const email = d?.email || STT_USER.email;
    const name  = d?.name  || "User";

    if (!email) { showToast("No email found on your account.", "error"); btn.disabled = false; btn.innerHTML = "Verify Phone"; return; }

    generatedOtp = Math.floor(100000 + Math.random() * 900000).toString();
    otpExpiry    = Date.now() + 5 * 60 * 1000;

    await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, { to_name: name, to_email: email, otp_code: generatedOtp, phone });
...
function closeOtpModal() { document.getElementById("otpModal").classList.remove("stt-modal-active"); }

document.getElementById("otpModalClose")?.addEventListener("click", closeOtpModal);
document.getElementById("otpModal")?.addEventListener("click", e => { if (e.target.id === "otpModal") closeOtpModal(); });

document.querySelectorAll(".stt-otp-box").forEach((box, i, boxes) => {
  box.addEventListener("input", () => { box.value = box.value.replace(/\D/g, "").slice(0, 1); if (box.value && i < boxes.length - 1) boxes[i + 1].focus(); });
...
function startResendTimer() {
  ...
  resendInterval = setInterval(() => {
    seconds--;
    if (timerEl) timerEl.textContent = `Resend in ${seconds}s`;
    if (seconds <= 0) { clearInterval(resendInterval); if (resendBtn) resendBtn.disabled = false; if (timerEl) timerEl.textContent = ""; }
  }, 1000);
}

document.getElementById("resendOtpBtn")?.addEventListener("click", () => { document.getElementById("verifyPhoneBtn")?.click(); });
...
document.getElementById("confirmOtpBtn")?.addEventListener("click", async () => {
  ...
  if (!generatedOtp)  { setEl("otpError", "Session expired. Request a new OTP."); return; }
  if (Date.now() > otpExpiry) { setEl("otpError", "OTP has expired. Please request a new one."); generatedOtp = null; return; }
  if (otp !== generatedOtp) {
...
function showSuccess(id) { const el = document.getElementById(id); if (!el) return; el.style.display = "flex"; setTimeout(() => el.style.display = "none", 3500); }
...
const NOTIF_KEYS = ["notifDeposit","notifWithdraw","notifProfit","notifReferral","notifPremReferral","notifMaturity","notifActivation","notifLogin","notifLoan","notifSecurity","notifPromo"];
...
async function loadNotifPrefs(uid) {
  try {
    ...
    NOTIF_KEYS.forEach(key => { const el = document.getElementById(key); if (!el) return; el.checked = key in prefs ? prefs[key] : key !== "notifPromo"; });
...
document.getElementById("saveNotifBtn")?.addEventListener("click", async () => {
  ...
  const prefs = {};
  NOTIF_KEYS.forEach(key => { const el = document.getElementById(key); if (el) prefs[key] = el.checked; });
  try {
  ...
  } catch (err) { console.error(err); showToast("Failed to save preferences.", "error"); }
  btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Save Preferences';
});

document.getElementById("notifAllOffBtn")?.addEventListener("click", () => {
  NOTIF_KEYS.forEach(key => { const el = document.getElementById(key); if (el) el.checked = false; });
...
async function loadPrivacySettings(uid) {
  try {
    ...
    PRIVACY_KEYS.forEach(key => { const el = document.getElementById(key); if (!el) return; el.checked = key in prefs ? prefs[key] : key !== "privMarketing"; });
...
document.getElementById("savePrivacyBtn")?.addEventListener("click", async () => {
  ...
  btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Saving...';
  const prefs = {};
  PRIVACY_KEYS.forEach(key => { const el = document.getElementById(key); if (el) prefs[key] = el.checked; });
  try {
    await updateDoc(doc(db, "users", STT_USER.uid), { privacySettings: prefs });
    showSuccess("privacySuccess");
    showToast("Privacy settings saved.", "success");
  } catch (err) { console.error(err); showToast("Failed to save. Try again.", "error"); }
  btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Save Privacy Settings';
});
...
function applyHideBalance(hide) {
  ["totalBalance","invAvailBal","currentBalance","availableBalance"].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.style.filter = hide ? "blur(6px)" : "";
  });
}

function applyCompactMode(on) { document.body.classList.toggle("stt-compact", on); }
...
document.getElementById("saveDisplayBtn")?.addEventListener("click", async () => {
  ...
  btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Save Display Settings';
});
...
document.getElementById("saveTxBtn")?.addEventListener("click", async () => {
  ...
  btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Save Transaction Settings';
});