...
document.getElementById("wdrSubmitBtn").addEventListener("click", async () => {
  ...
  if (todayTotal >= WITHDRAW_MAX) {
    errEl.textContent = `Daily limit of GHS ${WITHDRAW_MAX.toLocaleString()} reached. You have already withdrawn ${fmtGHS(todayTotal)} today.`;
    return;
  }
  if (todayTotal + amount > WITHDRAW_MAX) {
    errEl.textContent = `This would exceed your daily limit. Maximum you can withdraw now is ${fmtGHS(WITHDRAW_MAX - todayTotal)}.`;
    return;
  }
