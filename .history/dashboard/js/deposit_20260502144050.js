...
onAuthStateChanged(auth, async (user) => {
  ...
  if (DEP_SETTINGS.maintenanceMode) {
    document.getElementById("depError").textContent = "The platform is currently under maintenance. Please try again later.";
  ...
  if (!DEP_SETTINGS.depositsEnabled) {
    ...
    if (errEl) errEl.textContent = "Deposits are temporarily disabled. Please try again later.";
...
document.getElementById("depositBtn").addEventListener("click", async () => {
  ...
  if (!DEP_SETTINGS) DEP_SETTINGS = await getPlatformSettings();

  if (DEP_SETTINGS.maintenanceMode) {
    errEl.textContent = "The platform is currently under maintenance. Please try again later.";
    return;
  }

  if (!DEP_SETTINGS.depositsEnabled) {
    errEl.textContent = "Deposits are temporarily disabled. Please try again later.";
    return;
  }
...
async function handleReferralCredit(uData) {
  try {
    if (DEP_SETTINGS && !DEP_SETTINGS.referralsEnabled) return;
