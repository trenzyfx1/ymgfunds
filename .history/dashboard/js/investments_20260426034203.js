...
// REPLACE the previous JS_ADDITIONS you pasted before
// ═══════════════════════════════════════════════════════
...
let INV_PW_ACTION = null; // "invest" or "activate"
let INV_PW_PLAN   = null; // plan name when action is "invest"

// ── Replace invest now buttons with password-first flow ─
document.querySelectorAll(".inv-cta-btn").forEach(oldBtn => {
  ...
  newBtn.addEventListener("click", () => {
    ...
    // Plan unlocked — ask password before invest
    INV_PW_ACTION = "invest";
    INV_PW_PLAN   = plan;
    INV_PLAN      = plan; // ← CRITICAL: set the original variable too
    pwModalOpen();
  });
});

// ── Replace activation banner buttons too ──────────────
["stdActBtn", "premActBtn"].forEach(id => {
  const el = document.getElementById(id);
  if (!el) return;
  const clone = el.cloneNode(true);
  el.parentNode.replaceChild(clone, el);
  clone.addEventListener("click", () => {
    INV_ACT_TIER  = id === "stdActBtn" ? "standard" : "premium";
    INV_PW_ACTION = "activate";
    pwModalOpen();
  });
});
...
document.getElementById("pwModalConfirm").addEventListener("click", async () => {
  ...
  try {
    ...
    if (INV_PW_ACTION === "activate") {
      // Populate and open activation modal
      const fee = INV_ACT_TIER === "standard" ? 500 : 1000;
      document.getElementById("actModalTitle").textContent =
        INV_ACT_TIER === "standard" ? "Activate Standard Plans" : "Activate Premium Plans";
      document.getElementById("actModalDesc").textContent =
        `Pay a one-time GHS ${fee} fee to unlock all ${INV_ACT_TIER} plans.`;
      ...
      INV_PLAN   = plan; // make sure original var is set
      const cfg  = INV_PLANS[plan];
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (lnEl && !lnEl.value) lnEl.value = data.name  || "";
  if (leEl && !leEl.value) leEl.value = data.email || user.email || "";
  if (lpEl && !lpEl.value) lpEl.value = data.phone || "";
});
...
// ── Loan Form ──────────────────────────────────────────
// FORMSPREE — replace YOUR_FORM_ID below with the ID from formspree.io
  ...
  loanSubmitBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "loanRequests"), {
        ...
        amount: parseFloat(amount), purpose,
        status: "under_review",
      ...
      if (FORMSPREE_URL.includes("mnjlrbgn")) {
        console.warn("Formspree URL not set yet — email not sent");
      } else {
        await fetch(FORMSPREE_URL, {
          ...
          body: JSON.stringify({
            ...
            purpose,
            subject: `New Loan Request from ${name}`
          })
        });
      }
