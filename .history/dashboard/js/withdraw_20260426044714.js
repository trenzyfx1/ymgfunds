...
document.getElementById("wdrSubmitBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    await fetch(FORMSPREE_URL, {
      ...
      body: JSON.stringify({
        subject:        `💸 Withdrawal Request — ${name} — ${fmtGHS(amount)}`,
        name,
        email,
        phone,
        amount:         fmtGHS(amount),
        fee:            fmtGHS(WITHDRAW_FEE),
        you_receive:    fmtGHS(netAmount),
        method,
        payment_details: paymentDetails,
        reference,
        request_date:   now.toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }),
        id_document:    idBase64  // base64 image — viewable in email
      })
    });
