...
const MIN_BALANCE    = 50; // GHS 50 must always remain
...
let WDR_ACCT_LOCKED = false; // true if first withdrawal already done
...
onAuthStateChanged(auth, async (user) => {
  ...
  onSnapshot(doc(db, "users", user.uid), async (snap) => {
    ...
    // Check if account is locked (first withdrawal done)
...
document.getElementById("wdrAmount").addEventListener("input", () => {
  ...
  document.getElementById("wdrSumRemaining").textContent = fmtGHS(remaining);

  // Warn if remaining would go below minimum
  const remEl = document.getElementById("wdrSumRemaining");
  if (remEl) remEl.style.color = remaining < MIN_BALANCE ? "#dc2626" : "var(--text-main)";
...
function openPwModal(amount) {
  ...
  document.getElementById("wdrPwSumAmt").textContent  = fmtGHS(amount);
  document.getElementById("wdrPwSumNet").textContent  = fmtGHS(net > 0 ? net : 0);
  document.getElementById("wdrPwSumAcct").textContent = acctSummary;
...
document.getElementById("wdrPwConfirm").addEventListener("click", async () => {
  ...
  try {
    ...
    // Password correct — close modal and submit
    document.getElementById("wdrPwModal").classList.remove("inv-modal-active");
...
async function processWithdrawal() {
  ...
  try {
    ...
    // Build update object — lock account on first withdrawal
    ...
    // Save to top-level withdrawalRequests
