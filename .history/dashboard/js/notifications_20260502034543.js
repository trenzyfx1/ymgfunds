...
// separate listener ONLY for unread count — no limit so it counts ALL unread
function listenUnreadCount() {
  if (NOTIF_COUNT_UNSUB) NOTIF_COUNT_UNSUB();
  ...
  NOTIF_COUNT_UNSUB = onSnapshot(q, (snap) => {
...
function timeAgo(ts) {
  ...
  // serverTimestamp() not yet resolved — show Just now
  ...
  // Use UTC-aware diff — both Date.now() and Firestore timestamps are UTC
  // If the diff comes out negative or impossibly large, it's a clock issue
  const now  = Date.now();
  const diff = now - ms;

  // New notification not yet synced or device clock ahead
