...
document.getElementById("verifyEmailBtn").addEventListener("click", async () => {
  ...
  if (now - lastEmailSentTime < 60000) {
    showToast("Wait 60 seconds before sending again.", "error");
    return;
  }

  if (!currentUser) {
    showToast("User not loaded. Refresh page.", "error");
    return;
  }

  if (currentUser.emailVerified) return;

  lastEmailSentTime = now; // 🔥 SAVE TIME
...
document.getElementById("verifyPhoneBtn").addEventListener("click", async () => {
  ...
  if (!phone.startsWith("+")) {
    showToast("Use format like +2348012345678", "error");
    return;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (changeBtn) {
    changeBtn.addEventListener("click", async () => {
      ...
      changeBtn.innerHTML = '<i class="fa-solid fa-key"></i> Update Password';
    });
  }
