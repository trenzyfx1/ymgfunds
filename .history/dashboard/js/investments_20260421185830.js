...
const PLANS = {
  "Starter Savings": {
    monthlyRate: 0.01,       // 1%/month
    dailyRate: 0.01 / 30,
    minDeposit: 50,
    duration: null,           // no lock-in
    label: "Flexible"
  },
  "Fixed Deposit": {
...
document.querySelectorAll(".gp-duration-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    currentPlan = `Growth Plus ${duration}`;
...
document.getElementById("investModal").addEventListener("click", (e) => {
  if (e.target === document.getElementById("investModal"))
    document.getElementById("investModal").classList.remove("active");
});
...
document.getElementById("confirmInvestBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    // Deduct from user balance, add to invested
    const userRef = doc(db, "users", currentUser.uid);
    const userSnap = await getDoc(userRef);
    const userData = userSnap.data();
    const newBalance = (userData.balance || 0) - amountVal;
    const newInvested = (userData.invested || 0) + amountVal;
    const newActivePlans = (userData.activePlans || 0) + 1;
    ...
    snap.forEach(docSnap => {
      ...
      tbody.innerHTML += `
        <tr>
          ...
          <td>GHS ${inv.amount?.toLocaleString("en-GH", { minimumFractionDigits: 2 })}</td>
          <td>${startDate}</td>
          <td>${maturity}</td>
...
// ── DAILY PROFIT ENGINE ────────────────────────
// Runs on page load — checks each investment and adds profit if 24hrs have passed
async function runDailyProfitEngine() {
  try {
    ...
    for (const docSnap of snap.docs) {
      ...
      // Calculate how many full days have passed since last profit
      ...
      // Simple interest: profit = amount × dailyRate × days
      ...
      if (inv.maturityDate?.seconds) {
        const maturity = new Date(inv.maturityDate.seconds * 1000);
        if (now >= maturity) newStatus = "matured";
      }
    ...
    // Add total new profit to user's balance and profit fields
    if (totalNewProfit > 0) {
      const userRef = doc(db, "users", currentUser.uid);
      const userSnap = await getDoc(userRef);
      const userData = userSnap.data();
