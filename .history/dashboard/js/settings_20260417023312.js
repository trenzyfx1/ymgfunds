...
onAuthStateChanged(auth, async (user) => {
  ...
  if (userDoc.exists()) {
    ...
    // Avatar initials — topbar + settings card
...
document.getElementById("verifyEmailBtn").addEventListener("click", async () => {
  ...
  try {
    // Redirect back to this settings page after email is verified
    ...
    // Poll every 5s — auto-update UI when user clicks the link
...
// Confirm OTP
// ── When you're ready: replace the isValid line with your SMS API verification call ──
document.getElementById("confirmOtpBtn").addEventListener("click", async () => {
  ...
  const isValid = otp === "123456"; // ← replace with real SMS API check
